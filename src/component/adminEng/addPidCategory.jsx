...
const AddCategory = ({ data }) => {
  ...
  const addCategoryDocument = async () => {
    try {
      const docRef = await addDoc(collection(db, "pidCategoryEn"), {
        uid: "",
        categoryName: selectedCategory, // Використовуйте вибрану категорію
        pidCategoryName: catName,
      });
      const newDocId = docRef.id;
      await updateDoc(doc(db, "pidCategoryEn", newDocId), {
        uid: newDocId,
      });
...
export default withFirebaseCollection("categoryEn")(AddCategory);
