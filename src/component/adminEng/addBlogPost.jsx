...
const AddBlogPost = ({ data }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const firestore = getFirestore();
      const docRef = await addDoc(collection(firestore, "blogEn"), blogData);
      console.log("Document written with ID: ", docRef.id);

      // Перезавантаження сторінки після успішного завантаження
...
export default withFirebaseCollection("categoryBlogEn")(AddBlogPost);
